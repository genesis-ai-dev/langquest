...
export class AttachmentStateManager {
    ...
    markDownloadOperationStart(): void {
        console.log('🚫 [ATTACHMENT STATE] Download operation started - pausing attachment updates');
        ...
        this.downloadOperationTimer = setTimeout(() => {
            console.log('⏰ [ATTACHMENT STATE] Download operation timeout - resuming attachment updates');
    ...
    markDownloadOperationComplete(): void {
        console.log('✅ [ATTACHMENT STATE] Download operation completed - resuming attachment updates');
        ...
        if (this.pendingUpdates.size > 0) {
            console.log(`🔄 [ATTACHMENT STATE] Processing ${this.pendingUpdates.size} pending updates: ${Array.from(this.pendingUpdates).join(', ')}`);
            const pendingSources = Array.from(this.pendingUpdates);
            this.pendingUpdates.clear();

            // Process the most important update (quest_downloads takes priority)
            const priorityOrder = ['quest_downloads', 'asset_downloads', 'quest_asset_links', 'asset_content', 'translations'];
            ...
            if (sortedSources.length > 0) {
                ...
                console.log(`🎯 [ATTACHMENT STATE] Processing priority update: ${sortedSources[0]}`);
            }
        }
    }
    ...
    async getUnifiedPermanentAttachmentIds(): Promise<string[]> {
        try {
            ...
            if (!currentUser?.id) {
                console.log('[ATTACHMENT STATE] No current user, returning empty attachment list');
                return [];
            }

            console.log('[ATTACHMENT STATE] 🔍 Collecting unified permanent attachment IDs...');

            // 1. Get directly downloaded assets
            const directAssets = await this.db.query.asset.findMany({
                where: (asset, { and, eq }) => and(
                    eq(asset.download_profiles, [currentUser.id])
                ),
                columns: { id: true, images: true }
            });

            console.log(`[ATTACHMENT STATE] Found ${directAssets.length} directly downloaded assets`);
            ...
            console.log(`[ATTACHMENT STATE] Found ${downloadedQuests.length} downloaded quests`);

            // Get quest-asset links for downloaded quests
            const questAssetLinks = downloadedQuests.length > 0
                ? await this.db.query.quest_asset_link.findMany({
                    where: (link, { inArray }) => inArray(link.quest_id, downloadedQuests.map(q => q.id)),
                    columns: { asset_id: true }
                })
                : [];

            console.log(`[ATTACHMENT STATE] Found ${questAssetLinks.length} assets in downloaded quests`);

            // Get asset details for quest assets
            const questAssets = questAssetLinks.length > 0
                ? await this.db.query.asset.findMany({
                    where: (asset, { inArray }) => inArray(asset.id, questAssetLinks.map(l => l.asset_id)),
                    columns: { id: true, images: true }
                })
                : [];
            ...
            console.log(`[ATTACHMENT STATE] ✅ Collected ${uniqueAttachmentIds.length} unique permanent attachment IDs from ${processedAssetIds.size} assets`);

            // Log sources breakdown
            let totalImages = 0, totalContent = 0, totalTranslations = 0;
            assetSources.forEach(sources => {
                sources.forEach(source => {
                    switch (source.type) {
                        case 'asset_images': totalImages += source.attachmentIds.length; break;
                        case 'asset_content': totalContent += source.attachmentIds.length; break;
                        case 'translations': totalTranslations += source.attachmentIds.length; break;
                    }
                });
            });

            console.log(`[ATTACHMENT STATE] 📊 Breakdown: ${totalImages} images, ${totalContent} content audio, ${totalTranslations} translation audio`);

            return uniqueAttachmentIds;
        } catch (error) {
            console.error('[ATTACHMENT STATE] ❌ Error collecting attachment IDs:', error);
            return [];
        }
    }
    ...
    async getAttachmentIdsForAssets(assetIds: string[]): Promise<string[]> {
        ...
        try {
            console.log(`[ATTACHMENT STATE] 🔍 Getting attachments for specific assets: ${assetIds.join(', ')}`);
            ...
            const uniqueAttachmentIds = [...new Set(allAttachmentIds)];
            console.log(`[ATTACHMENT STATE] ✅ Found ${uniqueAttachmentIds.length} attachments for ${assetIds.length} assets`);

            return uniqueAttachmentIds;
        } catch (error) {
            console.error('[ATTACHMENT STATE] ❌ Error getting asset attachment IDs:', error);
            return [];
        }
    }
 ...
 */
    updateWithDebounce(onUpdate: (ids: string[]) => void, triggerSource: string): void {
        ...
        if (this.downloadOperationInProgress) {
            console.log(`🚫 [ATTACHMENT STATE] Skipping update (${triggerSource}) - download operation in progress`);
        ...
        const isDownloadRelated = ['quest_downloads', 'asset_downloads', 'quest_asset_links'].includes(triggerSource);
        const debounceTime = isDownloadRelated ? 5000 : 1000; // 5 seconds for download operations, 1 second for others

        console.log(`⏱️ [ATTACHMENT STATE] Using ${debounceTime}ms debounce for trigger: ${triggerSource}`);
     ...
     */
    private async updateAttachmentState(onUpdate: (ids: string[]) => void, triggerSource: string): Promise<void> {
        if (this.updateInProgress) {
            console.log(`🔒 [ATTACHMENT STATE] Update already in progress, skipping trigger from ${triggerSource}`);
            return;
        }
        ...
        try {
            console.log(`🔄 [ATTACHMENT STATE] Starting attachment state update (triggered by: ${triggerSource})`);
            ...
            if (hasChanged) {
                ...
                console.log(`🔄 [ATTACHMENT STATE] Previous: ${this.unifiedAttachmentIds.size} attachments`);
                console.log(`🔄 [ATTACHMENT STATE] New: ${newAttachmentIds.length} attachments`);

                // Show what changed
                const previousIds = Array.from(this.unifiedAttachmentIds);
                const added = newAttachmentIds.filter(id => !this.unifiedAttachmentIds.has(id));
                const removed = previousIds.filter(id => !newAttachmentSet.has(id));

                if (added.length > 0) {
                    console.log(`🔄 [ATTACHMENT STATE] ➕ Added ${added.length} attachments: ${added.slice(0, 5).join(', ')}${added.length > 5 ? '...' : ''}`);
                }
                if (removed.length > 0) {
                    console.log(`🔄 [ATTACHMENT STATE] ➖ Removed ${removed.length} attachments: ${removed.slice(0, 5).join(', ')}${removed.length > 5 ? '...' : ''}`);
                }
                ...
                console.log(`🔄 [ATTACHMENT STATE] 📤 Calling PowerSync onUpdate with ${newAttachmentIds.length} attachments...`);
                onUpdate(newAttachmentIds);
                console.log(`🔄 [ATTACHMENT STATE] ✅ PowerSync onUpdate completed`);
            } else {
                console.log(`🔄 [ATTACHMENT STATE] ⏭️ No change in attachment list (${newAttachmentIds.length} attachments), skipping PowerSync update`);
            }

            const updateDuration = Date.now() - updateStartTime;
            console.log(`🔄 [ATTACHMENT STATE] Update completed in ${updateDuration}ms`);
 ...
 */
    processPendingUpdates(onUpdate: (ids: string[]) => void): void {
        if (this.pendingUpdates.size > 0 && !this.downloadOperationInProgress) {
            ...
            if (sortedSources.length > 0) {
                console.log(`🎯 [ATTACHMENT STATE] Processing deferred update: ${sortedSources[0]}`);
