...
export async function fetchServerSchemaVersion(
  ...
  try {
    ...
    if (error) {
      console.error('[SchemaVersionService] Failed to fetch server schema:', error);
    ...
    console.log('[SchemaVersionService] Server schema version:', schemaInfo.schema_version);

    return schemaInfo.schema_version;
  } catch (error) {
    console.error('[SchemaVersionService] Error fetching server schema:', error);
    throw error;
  }
}
...
export async function getLocalSchemaVersion(db: DrizzleDB): Promise<string> {
  ...
  if (minLocalVersion !== null) {
    console.log('[SchemaVersionService] Using local-only records version:', minLocalVersion);
    return minLocalVersion;
  }

  // No local-only records, use the code's schema version
  console.log('[SchemaVersionService] No local-only records, using APP_SCHEMA_VERSION:', APP_SCHEMA_VERSION);
...
export async function checkAppUpgradeNeeded(
  ...
  try {
    ...
    if (comparison === 0) {
      console.log('[SchemaVersionService] ✓ Schema versions match - no upgrade needed');
      return;
    }
    ...
    if (comparison < 0) {
      ...
      throw new AppUpgradeNeededError('server_ahead', localVersion, serverVersion);
    }
    ...
    throw new AppUpgradeNeededError('server_behind', localVersion, serverVersion);
  } catch (error) {
