 ...
 * - Look up languoids from both synced anmod local tables
 */
...
export const migration_2_3_to_2_4: Migration = {
  ...
  async migrate(db, onProgress) {
    console.log(
      '[Migration 2.3→2.4] Migrating languoid preferences from Language to Languoid format...'
    );
    ...
    try {
      ...
      if (!storedData) {
        console.log(
          '[Migration 2.3→2.4] No Zustand store data found, skipping'
        );
        return;
      }
      ...
      const migrateLanguoidToLanguoid = async (lang: unknown) => {
        ...
        try {
          ...
          if (languoidSyncedExists) {
            ...
            if (results.length > 0 && results[0]?.data) {
              ...
              console.log(
                `[Migration 2.3→2.4] Migrated languoid ${langObj.id} to languoid ${(languoid as { id?: string }).id} (synced)`
              );
              return languoid;
            }
          }
          ...
          if (languoidLocalExists) {
            ...
            if (results.length > 0 && results[0]?.data) {
              const languoid = JSON.parse(results[0].data);
              console.log(
                `[Migration 2.3→2.4] Migrated languoid ${langObj.id} to languoid ${(languoid as { id?: string }).id} (local)`
              );
              return languoid;
            }
          }
          ...
          console.warn(
            `[Migration 2.3→2.4] Error finding languoid by ID ${langObj.id}:`,
            error
          );
        }
        ...
        console.warn(
          `[Migration 2.3→2.4] Could not migrate languoid ${langObj.id}, clearing`
        );
        return null;
      };
      ...
      if (state.uiLanguage) {
        ...
        if (migrated) {
          ...
          needsUpdate = true;
          console.log('[Migration 2.3→2.4] Migrated uiLanguage to uiLanguoid');
        } else {
          delete state.uiLanguage;
      ...
      if (state.savedLanguage) {
        ...
        if (migrated) {
          ...
          needsUpdate = true;
          console.log(
            '[Migration 2.3→2.4] Migrated savedLanguage to savedLanguoid'
          );
        } else {
      ...
      if (needsUpdate) {
        ...
        console.log('[Migration 2.3→2.4] ✓ Migrated languoid preferences');
      }
    } catch (error) {
      console.warn(
        '[Migration 2.3→2.4] Could not migrate Zustand languoid preferences:',
        error
      );
