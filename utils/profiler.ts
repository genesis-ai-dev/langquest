...
class DebugProfiler {
  ...
  startNavigation(route: string) {
    ...
    console.log(
      `🚀 [NAV] Starting navigation to: ${route} at ${timestamp.toFixed(2)}ms`
    );
  }
  ...
  endNavigation(route: string) {
    ...
    console.log(
      `🏁 [NAV] Navigation to ${route} completed in ${duration.toFixed(2)}ms`
    );

    if (duration > 500) {
      console.warn(
        `⚠️ [NAV] Slow navigation detected: ${route} took ${duration.toFixed(2)}ms`
      );
    }
  }
  ...
  trackRender(componentName: string, props?: Record<string, unknown>) {
    ...
    if (count <= 5 || count % 10 === 0) {
      console.log(
        ...
        props ? { propKeys: Object.keys(props) } : ''
      );
    }

    if (count > 20) {
      console.warn(
        `⚠️ [RENDER] ${componentName} has rendered ${count} times - potential issue!`
      );
    }
  }
  ...
  trackEffect(
    ...
    if (duration && duration > 16) {
      // More than one frame
      console.warn(
        `⚠️ [EFFECT] Slow useEffect in ${componentName}:${effectId} took ${duration.toFixed(2)}ms`
      );
    }
  ...
  trackStateUpdate(
    ...
    const recentSameUpdates = this.recentStateUpdates
      .filter(
        (u) => u.componentName === componentName && u.stateName === stateName
      )
      .filter((u) => performance.now() - u.timestamp < 100); // Last 100ms

    if (recentSameUpdates.length > 5) {
      console.error(
        `🔥 [STATE] Rapid state updates detected in ${componentName}.${stateName} - ${recentSameUpdates.length} updates in 100ms!`
      );
    }
    ...
    const checkBlocking = () => {
      ...
      if (currentTime - lastSecondReset > 1000) {
        if (totalBlockingInLastSecond > 3000 || consecutiveBlocks > 20) {
          console.error(
            `🚨 [CIRCUIT BREAKER] Excessive blocking detected: ${totalBlockingInLastSecond.toFixed(1)}ms in 1s or ${consecutiveBlocks} consecutive blocks. Pausing profiler for 10 seconds.`
          );
          profilerPaused = true;
          setTimeout(() => {
      ...
      if (delta > 50 && delta < 10000) {
        // Ignore huge deltas that might be from pausing
        consecutiveBlocks++;
        ...
        console.warn(
          `🚫 [BLOCKING] Main thread blocked for ${delta.toFixed(2)}ms`
        );

        // Provide context about what might have caused it
        if (blockingEvent.activeQueries.length > 0) {
          console.warn(
            `   Active queries: ${blockingEvent.activeQueries.length}`
          );
        }
        if (blockingEvent.recentStateUpdates.length > 0) {
          console.warn(
            `   Recent state updates: ${blockingEvent.recentStateUpdates.join(', ')}`
          );
        }
        if (blockingEvent.recentEffects.length > 0) {
          console.warn(
            `   Recent effects: ${blockingEvent.recentEffects.join(', ')}`
          );
        }
        if (blockingEvent.memoryUsage && blockingEvent.memoryUsage > 100) {
          console.warn(
            `   Memory usage: ${blockingEvent.memoryUsage.toFixed(1)}MB`
          );
        }
        ...
        if (delta > 1000) {
          console.error(
            `🔥 [SEVERE BLOCKING] ${delta.toFixed(2)}ms - This is causing noticeable lag!`
          );
        ...
        if (consecutiveBlocks > 10) {
          console.error(
            `🔥 [BLOCKING PATTERN] ${consecutiveBlocks} consecutive blocks detected - investigate immediately!`
          );
  ...
  getStats() {
    ...
    return {
      ...
      memoryUsage: this.getMemoryUsage(),
      totalBlockingTime: this.blockingEvents.reduce(
  ...
  getDiagnosticReport() {
    ...
    return {
      summary: {
        ...
        averageBlockingTime:
          this.blockingEvents.length > 0
            ? this.blockingEvents.reduce(
                (sum, event) => sum + event.duration,
                0
      ...
      recentActivity: {
        effects: this.recentEffects.filter(
          (e) => now - e.timestamp < recentTime
        ),
        stateUpdates: this.recentStateUpdates.filter(
          (s) => now - s.timestamp < recentTime
        ),
        activeQueries: Array.from(this.activeQueries)
      },
