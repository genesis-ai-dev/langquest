/**
 * Extract waveform amplitude data from a WAV audio file.
 *
 * Reads the raw PCM samples, divides them into `barCount` windows,
 * computes the RMS amplitude of each window, and returns normalised
...
export function extractWaveformFromBuffer(
  ...
  const view = new DataView(buffer);
  const framesPerBar = Math.max(1, Math.floor(totalFrames / barCount));
