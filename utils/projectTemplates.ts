...
// Bible book structure with chapter and verse counts
const BIBLE_BOOKS: {
    name: string;
    chapters: number[];
}[] = [
...
export const BIBLE_TEMPLATE: ProjectTemplate = {
    ...
    questCount: BIBLE_BOOKS.reduce((total, book) => total + book.chapters.length, 0), // ~1189 chapters
    assetCount: BIBLE_BOOKS.reduce((total, book) =>
    ...
    createAssets: (sourceLanguageId: string, _questId: string, questName: string): AssetTemplate[] => {
        ...
        if (!bookName || !chapterStr || isNaN(chapterNumber)) return [];
