    ...
    try {
        await system.db.transaction(async (tx) => {
            ...
            const updateProgress = (message: string) => {
                ...
                console.log(`🗑️ [Offload] Step ${currentStep}/${totalSteps}: ${message}`);
            ...
            if (verifiedIds.voteIds.length > 0) {
                await tx.delete(drizzleSchemaLocal.vote_local)
                    .where(inArray(drizzleSchemaLocal.vote_local.id, verifiedIds.voteIds));
            ...
            if (verifiedIds.assetTagLinkIds.length > 0) {
                ...
                for (const { assetId, tagId } of assetTagPairs) {
                    await tx.delete(drizzleSchemaLocal.asset_tag_link_local)
                        .where(
                            eq(drizzleSchemaLocal.asset_tag_link_local.asset_id, assetId)
                        );
                }
                console.log(`✅ [Offload] Deleted ${assetTagPairs.length} asset-tag links`);
            }
            ...
            if (verifiedIds.questTagLinkIds.length > 0) {
                ...
                for (const { questId: qId, tagId } of questTagPairs) {
                    await tx.delete(drizzleSchemaLocal.quest_tag_link_local)
                        .where(
                            eq(drizzleSchemaLocal.quest_tag_link_local.quest_id, qId)
                        );
                }
                console.log(`✅ [Offload] Deleted ${questTagPairs.length} quest-tag links`);
            }
            ...
            if (verifiedIds.assetContentLinkIds.length > 0) {
                await tx.delete(drizzleSchemaLocal.asset_content_link_local)
                    .where(inArray(drizzleSchemaLocal.asset_content_link_local.id, verifiedIds.assetContentLinkIds));
                console.log(`✅ [Offload] Deleted ${verifiedIds.assetContentLinkIds.length} asset-content links`);
            }
            ...
            if (verifiedIds.attachmentIds.length > 0) {
                ...
                for (const attachmentId of verifiedIds.attachmentIds) {
                    try {
                        ...
                        console.warn(`⚠️ [Offload] Failed to delete attachment ${attachmentId}:`, error);
                    }
                }
                console.log(`✅ [Offload] Cleaned up ${verifiedIds.attachmentIds.length} attachments`);
            }

            // Step 7: Delete assets
            updateProgress('Deleting assets...');
            if (verifiedIds.assetIds.length > 0) {
                await tx.delete(drizzleSchemaLocal.asset_local)
                    .where(inArray(drizzleSchemaLocal.asset_local.id, verifiedIds.assetIds));
                console.log(`✅ [Offload] Deleted ${verifiedIds.assetIds.length} assets`);
            }
            ...
            if (verifiedIds.questAssetLinkIds.length > 0) {
                ...
                for (const { questId: qId, assetId } of questAssetPairs) {
                    await tx.delete(drizzleSchemaLocal.quest_asset_link_local)
                        .where(
                            eq(drizzleSchemaLocal.quest_asset_link_local.quest_id, qId)
                        );
                }
                console.log(`✅ [Offload] Deleted ${questAssetPairs.length} quest-asset links`);
            }

            // Step 9: Delete the quest itself
            updateProgress('Deleting quest...');
            await tx.delete(drizzleSchemaLocal.quest_local)
            ...
            console.log('⏭️ [Offload] Skipping languages (likely shared across projects)');
...
async function removeFromDownloadProfile(questId: string): Promise<void> {
    try {
        ...
        if (!userId) {
            console.warn('⚠️ [Offload] No user ID found, skipping download profile update');
            return;
        }

        // Get current profile
        const { data: profile, error: fetchError } = await system.supabaseConnector.client
            .from('profile')
        ...
        const updatedDownloads = currentDownloads.filter((id: string) => id !== questId);
        ...
        if (updateError) {
            console.warn('⚠️ [Offload] Could not update download profile:', updateError);
        } else {
