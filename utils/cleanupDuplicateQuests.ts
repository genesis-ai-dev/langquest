...
export async function cleanupDuplicateQuests(projectId: string): Promise<{
    ...
    for (const group of duplicateGroups) {
        ...
        console.log(`  Keeping: ${keeper.id} (${keeper.source}, ${keeper.created_at})`);

        // Delete from local table
        const localToDelete = toDelete.filter(q => q.source === 'local').map(q => q.id);
        ...
        if (syncedDuplicates.length > 0) {
            console.warn(`    ⚠️  Found ${syncedDuplicates.length} synced duplicates - manual cleanup needed`);
        }
    }
 ...
 */
export async function previewDuplicateQuests(projectId: string): Promise<DuplicateGroup[]> {
