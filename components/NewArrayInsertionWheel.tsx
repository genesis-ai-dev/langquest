...
interface ArrayInsertionWheelProps {
  ...
  /** Called in real-time as the wheel spins (during scroll) */
  onIndexChanging?: (previousIndex: number, currentIndex: number) => void;
  /** Called when scroll ends and value is finalized */
  onIndexChanged?: (previousIndex: number, currentIndex: number) => void;
  rowHeight: number;
  className?: string;
...
function ArrayInsertionWheelInternal(
  ...
  // Track previous index for real-time updates (during scroll)
  const prevChangingIndexRef = React.useRef(clampedValue);
  // Track previous index for finalized updates (after scroll ends)
  const prevChangedIndexRef = React.useRef(clampedValue);

  // Handler for real-time updates during scroll
  const handleValueChanging = React.useCallback(
    ({ item }: { item: { value: number } }) => {
      const currentIndex = item.value;
      const previousIndex = prevChangingIndexRef.current;

      if (previousIndex !== currentIndex) {
        console.log(
          '🔄 Wheel changing (real-time):',
          previousIndex,
          '→',
          currentIndex
        );
        onIndexChanging?.(previousIndex, currentIndex);
        prevChangingIndexRef.current = currentIndex;
      }
    },
    [onIndexChanging]
  );

  // Handler for finalized updates after scroll ends
  const handleValueChanged = React.useCallback(
    ({ item }: { item: { value: number } }) => {
      const currentIndex = item.value;
      const previousIndex = prevChangedIndexRef.current;
      ...
      if (previousIndex !== currentIndex) {
        onIndexChanged?.(previousIndex, currentIndex);
        prevChangedIndexRef.current = currentIndex;
      }
    },
    [onChange, onIndexChanged]
  );

  // Use clampedValue directly as stableClampedValue
  const stableClampedValue = clampedValue;
