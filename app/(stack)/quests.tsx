...
const QuestCard: React.FC<{ quest: QuestWithRelations }> = ({ quest }) => {
  const difficulty = quest.tags
    .find((tag) => tag.name.startsWith('Difficulty:'))
    ?.name.split(':')[1]; // ****Needs attention****
...
export default function Quests() {
  ...
  const [selectedQuest, setSelectedQuest] = useState<QuestWithRelations | null>(
    null,
  );
  ...
  const applySorting = useCallback(
    (quests: QuestWithRelations[], sorting: SortingOption[]) => {
