 ...
 */
export async function saveRecording(params: SaveRecordingParams): Promise<string> {
    const { questId, projectId, targetLanguageId, userId, orderIndex, audioUri } = params;

    const newAssetId = String(uuid.v4());
    const assetName = String(orderIndex + 1).padStart(3, '0');

    console.log(`💾 Saving recording | name: ${assetName} | order_index: ${orderIndex}`);
...
export async function getNextOrderIndex(questId: string): Promise<number> {
    ...
    const maxOrder = Math.max(
        ...assets.map(a => typeof a.order_index === 'number' ? a.order_index : 0),
        -1
    );
