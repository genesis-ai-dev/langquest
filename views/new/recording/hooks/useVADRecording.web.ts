...
export function useVADRecording({
    ...
    // VAD state machine
    const vadStateRef = React.useRef<'idle' | 'onset' | 'speaking' | 'silence'>('idle');
    const silenceStartRef = React.useRef<number>(0);
    const lastEnergyRef = React.useRef<number>(0);

    const currentEnergy = energyResult?.smoothedEnergy ?? energyResult?.energy ?? 0;
            ...
            await new Promise<void>((resolve) => {
                recorder.onstop = () => {
                    ...
                    if (duration >= config.minSegmentDuration) {
                        ...
                        console.log(`📼 Web VAD: Segment complete (${duration}ms, ${blob.size} bytes)`);
            ...
            case 'silence':
                ...
                if (energy > confirmThreshold) {
                    ...
                    if (silenceDuration >= config.silenceDuration) {
                        console.log('🛑 Web VAD: Silence threshold reached, stopping segment');
                        vadStateRef.current = 'idle';
                        silenceStartRef.current = 0;
                        void stopSegment();
                    }
    ...
    }, [isVADActive, isActive, isManualRecording, isRecording, startEnergyDetection, stopEnergyDetection, stopSegment]);
