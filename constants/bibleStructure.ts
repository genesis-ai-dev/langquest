...
export const getMaxVersesInChapter = (
  bookId: string,
  chapter: number
): number => {
...
export const getNextVerse = (
  ...
  const maxVersesInChapter = getMaxVersesInChapter(
    reference.book,
    reference.chapter
  );
...
export const getPreviousVerse = (
  ...
  if (reference.chapter > 1) {
    ...
    const maxVersesInPreviousChapter = getMaxVersesInChapter(
      reference.book,
      previousChapter
    );
    return {
  ...
  if (currentBookIndex > 0) {
    ...
    if (previousBook) {
      const maxVersesInLastChapter = getMaxVersesInChapter(
        previousBook.id,
