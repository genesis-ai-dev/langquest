...
export function useAppNavigation() {
    ...
    const currentState = useMemo(() =>
        navigationStack[navigationStack.length - 1] || { view: 'projects' as AppView, timestamp: Date.now() }
        , [navigationStack]);

    // Navigation functions with instant state transitions
    const navigate = useCallback((newState: Partial<NavigationState>) => {
        profiler.startNavigation(`${newState.view}:${newState.projectId || newState.questId || newState.assetId || 'main'}`);
        ...
        profiler.endNavigation(`${newState.view}:${newState.projectId || newState.questId || newState.assetId || 'main'}`);
    ...
    const goToProject = useCallback((projectData: { id: string; name?: string }) => {
    ...
    const goToQuest = useCallback((questData: { id: string; project_id: string; name?: string }) => {
        // Track recently visited
        addRecentQuest({
            id: questData.id,
    ...
    const goToAsset = useCallback((assetData: {
        id: string;
        name?: string;
        projectId?: string;
        questId?: string;
    ...
    const breadcrumbs = useMemo(() => {
        ...
        if (state.view === 'projects') {
        ...
        } else if (state.view === 'assets' && state.projectName && state.questName) {
            crumbs.push({ label: 'Projects', onPress: goToProjects });
            crumbs.push({
                label: state.projectName,
                onPress: () => goToProject({ id: state.projectId!, name: state.projectName })
            });
            crumbs.push({ label: state.questName, onPress: undefined });
        } else if (state.view === 'asset-detail' && state.projectName && state.questName && state.assetName) {
            crumbs.push({ label: 'Projects', onPress: goToProjects });
            crumbs.push({
                label: state.projectName,
                onPress: () => goToProject({ id: state.projectId!, name: state.projectName })
            ...
            crumbs.push({
                ...
                onPress: () => goToQuest({ id: state.questId!, project_id: state.projectId!, name: state.questName })
            });
