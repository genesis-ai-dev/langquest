...
import { useQueuedAssetIdSet } from '@/hooks/useAssetGcQueue';
...
export function useAssetsByQuest(
  ...
  const queuedIds = useQueuedAssetIdSet();
  const filteredData = React.useMemo(() => {
    if (queuedIds.size === 0) return data;
    return {
      ...data,
      pages: data.pages.map((page) => ({
        ...page,
        data: page.data.filter((item) => !queuedIds.has(item.id))
      }))
    };
  }, [data, queuedIds]);
...
export function useLocalAssetsByQuest(
  ...
  const visibleAssets = React.useMemo(
    () =>
      queuedIds.size === 0
        ? normalized
        : normalized.filter((item) => !queuedIds.has(item.id)),
    [normalized, queuedIds]
  );

  const wrappedData = React.useMemo(() => {
    return {
      pages: [{ data: visibleAssets }],
      pageParams: [0]
    };
  }, [visibleAssets]);
