...
export interface VerificationState {
    ...
    progressSharedValues: {
        ...
        questAssetLinks: ReturnType<typeof useSharedValue<CategoryVerificationStatus>>;
        assets: ReturnType<typeof useSharedValue<CategoryVerificationStatus>>;
        assetContentLinks: ReturnType<typeof useSharedValue<CategoryVerificationStatus>>;
        votes: ReturnType<typeof useSharedValue<CategoryVerificationStatus>>;
        questTagLinks: ReturnType<typeof useSharedValue<CategoryVerificationStatus>>;
        assetTagLinks: ReturnType<typeof useSharedValue<CategoryVerificationStatus>>;
 ...
 */
export function useQuestOffloadVerification(questId: string): VerificationState {
    ...
    const questProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    const projectProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    const questAssetLinksProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    const assetsProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    const assetContentLinksProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    const votesProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    const questTagLinksProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    const assetTagLinksProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    const tagsProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    const languagesProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    const attachmentsProgress = useSharedValue<CategoryVerificationStatus>(initialStatus);
    ...
    const startVerification = useCallback(async () => {
        ...
        console.log(`🔍 [Offload Verification] Starting verification for quest: ${questId}`);
        setIsVerifying(true);
        setHasError(false);
        ...
        try {
            ...
            // ============================================================================
            console.log('🔍 [Offload Verification] Phase 1: Checking for pending uploads');
            ...
            const pendingRecordsResult = await system.powersync.getAll<{ count: number }>(
                `SELECT COUNT(*) as count FROM ps_crud`
            );
            const pendingDbRecords = pendingRecordsResult[0]?.count || 0;

            // Check attachment upload queue
            const pendingAttachmentsResult = await system.powersync.getAll<{ count: number }>(
            ...
            // ============================================================================
            console.log('🔍 [Offload Verification] Phase 2: Verifying database records');
            ...
            const [questResult, questAssetLinksResult, questTagLinksResult] = await Promise.all([
                // Verify quest
                (async () => {
                    try {
                        if (signal.aborted) return null;

                        questProgress.value = { count: 1, verified: 0, isVerifying: true, hasError: false };
                        ...
                        if (error || !data) {
                            console.error('🔍 [Offload Verification] Quest not found in cloud:', error);
                            questProgress.value = { count: 1, verified: 0, isVerifying: false, hasError: true };
                            setHasError(true);
                            return null;
                        }
                        ...
                        questProgress.value = { count: 1, verified: 1, isVerifying: false, hasError: false };
                        projectProgress.value = { count: 1, verified: 1, isVerifying: false, hasError: false };
                        updateTotal();

                        console.log(`✅ [Offload Verification] Quest verified in cloud: ${data.id}`);
                        return data;
                    } catch (error) {
                        console.error('🔍 [Offload Verification] Error verifying quest:', error);
                        questProgress.value = { count: 1, verified: 0, isVerifying: false, hasError: true };
                        projectProgress.value = { count: 1, verified: 0, isVerifying: false, hasError: true };
                        setHasError(true);
                        return null;
                    }
                })(),
                ...
                (async () => {
                    try {
                        ...
                        const localLinks = await system.db.query.quest_asset_link.findMany({
                        ...
                        ids.questAssetLinkIds = data.map((link) => `${link.quest_id}|${link.asset_id}`);
                        questAssetLinksProgress.value = {
                            count: linkCount,
                        ...
                        updateTotal();
                        console.log(`✅ [Offload Verification] Quest-asset links verified: ${data.length}/${linkCount}`);
                        return data;
                    } catch (error) {
                        console.error('🔍 [Offload Verification] Error verifying quest-asset links:', error);
                        questAssetLinksProgress.value = {
                ...
                (async () => {
                    try {
                        ...
                        ids.questTagLinkIds = data.map((link) => `${link.quest_id}|${link.tag_id}`);
                        questTagLinksProgress.value = {
                            count: linkCount,
                            verified: data.length,
                            isVerifying: false,
                        ...
                        updateTotal();
                        console.log(`✅ [Offload Verification] Quest-tag links verified: ${data.length}/${linkCount}`);
                        return data;
                    } catch (error) {
                        console.error('🔍 [Offload Verification] Error verifying quest-tag links:', error);
                        questTagLinksProgress.value = {
            ...
            const assetIds = questAssetLinksResult?.map((link) => link.asset_id) || [];
            ids.assetIds = assetIds;

            // Wave 2: Asset-dependent queries
            if (assetIds.length > 0) {
                const [assetsResult, assetContentLinksResult, assetTagLinksResult] = await Promise.all([
                    // Verify assets
                    (async () => {
                        try {
                            if (signal.aborted) return null;
                            ...
                            updateTotal();
                            console.log(`✅ [Offload Verification] Assets verified: ${data.length}/${assetCount}`);
                            return data;
                        } catch (error) {
                            console.error('🔍 [Offload Verification] Error verifying assets:', error);
                            assetsProgress.value = {
                    ...
                    (async () => {
                        try {
                            ...
                            const localLinks = await system.db.query.asset_content_link.findMany({
                                where: (link, { inArray }) => inArray(link.asset_id, assetIds)
                            });
                            ...
                            console.error('🔍 [Offload Verification] Error verifying asset-content links:', error);
                            assetContentLinksProgress.value = {
                    ...
                    (async () => {
                        try {
                            ...
                            const localLinks = await system.db.query.asset_tag_link.findMany({
                                where: (link, { inArray }) => inArray(link.asset_id, assetIds)
                            });
                            ...
                            ids.assetTagLinkIds = data.map((link) => `${link.asset_id}|${link.tag_id}`);
                            assetTagLinksProgress.value = {
                                count: linkCount,
                                verified: data.length,
                                isVerifying: false,
                            ...
                            updateTotal();
                            console.log(`✅ [Offload Verification] Asset-tag links verified: ${data.length}/${linkCount}`);
                            return data;
                        } catch (error) {
                            console.error('🔍 [Offload Verification] Error verifying asset-tag links:', error);
                            assetTagLinksProgress.value = {
                ...
                if (questResult?.project_id) {
                    try {
                        const { data: projectData, error } = await system.supabaseConnector.client
                            .from('project')
                        ...
                        if (!error && projectData) {
                            if (projectData.source_language_id) languageIds.add(projectData.source_language_id);
                            if (projectData.target_language_id) languageIds.add(projectData.target_language_id);
                        }
                    } catch (error) {
                        console.error('🔍 [Offload Verification] Error fetching project languages:', error);
                    }
                }

                assetsResult?.forEach((asset) => {
                    if (asset.source_language_id) languageIds.add(asset.source_language_id);
                });
                ...
                const [votesResult, tagsResult] = await Promise.all([
                    ...
                    (async () => {
                        try {
                            ...
                            updateTotal();
                            console.log(`✅ [Offload Verification] Votes verified: ${data.length}/${voteCount}`);
                            return data;
                        } catch (error) {
                            console.error('🔍 [Offload Verification] Error verifying votes:', error);
                            votesProgress.value = {
                    ...
                    (async () => {
                        try {
                            ...
                            updateTotal();
                            console.log(`✅ [Offload Verification] Tags verified: ${data.length}/${tagCount}`);
                            return data;
                        } catch (error) {
                            console.error('🔍 [Offload Verification] Error verifying tags:', error);
                            tagsProgress.value = {
                ...
                const audioFileIds = assetContentLinksResult
                    ...
                    try {
                        ...
                        for (const audioId of audioFileIds) {
                            ...
                            try {
                                ...
                                console.error(`🔍 [Offload Verification] Error checking attachment ${audioId}:`, error);
                            }
                        }
                        ...
                        console.error('🔍 [Offload Verification] Error verifying attachments:', error);
                        attachmentsProgress.value = {
                            count: attachmentCount,
                            verified: 0,
                            isVerifying: false,
                ...
                assetsProgress.value = { count: 0, verified: 0, isVerifying: false, hasError: false };
                assetContentLinksProgress.value = { count: 0, verified: 0, isVerifying: false, hasError: false };
                votesProgress.value = { count: 0, verified: 0, isVerifying: false, hasError: false };
                assetTagLinksProgress.value = { count: 0, verified: 0, isVerifying: false, hasError: false };
                tagsProgress.value = { count: 0, verified: 0, isVerifying: false, hasError: false };
                languagesProgress.value = { count: 0, verified: 0, isVerifying: false, hasError: false };
                attachmentsProgress.value = { count: 0, verified: 0, isVerifying: false, hasError: false };
            }
