  ...
  const { mutateAsync: findOrCreateBook, isPending } = useMutation({
    mutationFn: async (params: CreateBookParams): Promise<BookQuest> => {
      ...
      return await system.db.transaction(async (tx) => {
        ...
        // Note: All 66 Bible book tags are pre-seeded globally (project_id = null)

        console.log(
          `🔍 Looking for book "${bookName}" (${bookId}) in project ${projectId.substring(0, 8)}...`
        );

        // Find global Bible book tag (should always exist from seed data)
        ...
        // Link Bible tag for localization-proof identification
        // Note: All Bible book tags (66) are pre-seeded globally with fixed UUIDs
        // They should always exist in the database (synced from Supabase)

        const [bookTagToLink] = await tx
          .select()
          .from(tag)
          .where(
            and(
              eq(tag.key, BIBLE_TAG_KEYS.BOOK),
              eq(tag.value, bookId),
              isNull(tag.project_id) // Global Bible tags have no project_id
            )
          )
          .limit(1);
        ...
        if (!bookTagToLink) {
          console.warn(
            `⚠️  Bible book tag not found: ${bookId}. This should not happen - tags should be pre-seeded.`
          );
          console.warn(
            `   → Skipping tag link. Book quest created: ${newBook.id}`
          );
        } else {
          ...
          const existingLink = await tx
            ...
            .where(
              and(
                ...
                eq(quest_tag_link.tag_id, bookTagToLink.id)
              )
            )
            .limit(1);
