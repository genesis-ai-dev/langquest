...
- **Maestro** — `npm run maestro:local` runs [`maestro-local.ts`](maestro-local.ts). Flows, seeds, and device notes are in [`.maestro/`](../.maestro/README.md). `npm run maestro:local:assets` is the asset/audio-file subset (record-time upload, download, deletion queue). Failure dumps stay in `~/.maestro/tests/`.
