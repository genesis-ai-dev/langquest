...
function parseBibleMetadata(rawMetadata: unknown): BibleMetadata | null {
  ...
  return {
    book,
    chapter: chapter !== null && chapter > 0 ? Math.floor(chapter) : undefined
  };
}
  ...
  for (const [bookId, rootVersions] of rootGroupsByBook.entries()) {
    ...
    const subquests: DashboardSubquestItem[] = subquestGroups.map((group) => {
      ...
      const completedFlags =
        expectedVerses > 0
          ? new Array<boolean>(expectedVerses).fill(false)
          : [];
      ...
      if (expectedVerses > 0) {
        for (const asset of subquestAssets) {
          ...
          const clampedStart = Math.max(
            1,
            Math.min(expectedVerses, verseRange.from)
          );
          const clampedEnd = Math.max(
            1,
            Math.min(expectedVerses, verseRange.to)
          );
    ...
    const questName =
      asString(latestRoot?.name ?? null) ?? bookDef?.name ?? bookId;
