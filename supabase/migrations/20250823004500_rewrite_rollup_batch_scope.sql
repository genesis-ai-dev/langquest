-- purpose: eliminate CTE scope issues by computing batch count separately
-- notes: recompute the same batch CTE for UPDATE and for count to avoid referencing CTE in RETURNING
...
begin
  ...
  -- perform the rollup update using a local batch CTE
  with batch as (
  ...
  -- recompute v_rows separately with the same batch window
  with batch as (
    select qc.quest_id
    from public.quest_closure qc
    join public.quest q on q.id = qc.quest_id
