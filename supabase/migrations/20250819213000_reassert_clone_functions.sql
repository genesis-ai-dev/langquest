-- migration: reassert clone functions (perform_clone_step + batched quest closures)
-- purpose: ensure cloud has the full multi-stage clone implementation

-- Helper: rebuild up to p_limit quest closures for a project where last_updated is missing/stale
...
-- Full multi-stage clone stepper
...
declare
  ...
  lrec record;
begin
  -- Prevent concurrent workers per job
  ...
  if v_stage = 'seed_project' then
    if not exists (select 1 from public.map_project where job_id = p_job_id and src_id = v_root_project_id) then
      insert into public.project (
        name, description, source_language_id, target_language_id,
        active, creator_id, private, visible, download_profiles
      )
      select
        v_new_name,
        p.description,
  ...
  if v_stage = 'clone_quests' then
    ...
    for qrec in (
      ...
      insert into public.quest (
        ...
        name, description, project_id, active, creator_id, visible, download_profiles
      )
      values (
        gen_random_uuid(), now(), now(),
        qrec.name, qrec.description, v_dst_project_id, qrec.active, v_creator_id, qrec.visible, '{}'::uuid[]
  ...
  if v_stage = 'clone_assets' then
    ...
    for arec in (
      ...
      insert into public.asset (
        id, created_at, last_updated,
        name, images, active, creator_id, visible, download_profiles, source_language_id
      )
      values (
        gen_random_uuid(), now(), now(),
        arec.name, arec.images, arec.active, v_creator_id, arec.visible, '{}'::uuid[], arec.source_language_id
  ...
  -- Stage 3: clone asset_content_link (ACL)
  if v_stage = 'clone_acl' then
    ...
    for arec in (
      ...
      insert into public.asset_content_link (
        id, created_at, last_updated,
        asset_id, audio_id, text, active, download_profiles, source_language_id
      )
    ...
    for lrec in (
      ...
      then
        insert into public.quest_asset_link (
          quest_id, asset_id, active, created_at, last_updated, visible, download_profiles
        )
        ...
        where mq.job_id = p_job_id and ma.job_id = p_job_id
          and mq.src_id = lrec.quest_id and ma.src_id = lrec.asset_id
          and not exists (
  ...
  -- Stage 5: batched quest closure rebuild, then finalize project closure
