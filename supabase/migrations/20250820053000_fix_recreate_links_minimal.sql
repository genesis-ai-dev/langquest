-- migration: minimal fix for recreate_links batching & gating
-- purpose: prevent premature advance and ensure links are distributed correctly
...
begin
  ...
  -- Stage 4: recreate link tables (minimal fix)
  if v_stage = 'recreate_links' then
    v_rows := 0;
    for lrec in (
      select qal.*
      from public.quest_asset_link qal
      join public.quest sq on sq.id = qal.quest_id and sq.project_id = v_root_project_id
      where exists (select 1 from public.map_quest mq where mq.job_id = p_job_id and mq.src_id = qal.quest_id)
        ...
        and not exists (
          ...
          where mq.job_id = p_job_id and ma.job_id = p_job_id
            ...
            and exists (
              select 1 from public.quest_asset_link t
              where t.quest_id = mq.dst_id and t.asset_id = ma.dst_id
            )
        )
    ...
    -- Strict gating: advance only when dst equals src
    select count(*) into v_src_qal
    from public.quest_asset_link qal
    join public.quest sq on sq.id = qal.quest_id and sq.project_id = v_root_project_id;

    select count(*) into v_dst_qal
    from public.quest_asset_link qal
    join public.quest dq on dq.id = qal.quest_id and dq.project_id = v_dst_project_id;
    ...
    if coalesce(v_dst_qal,0) >= coalesce(v_src_qal,0) then
      ...
      return query select false, format('links complete %s/%s; advancing', v_dst_qal, v_src_qal);
    end if;
    return query select false, format('waiting links %s/%s', v_dst_qal, v_src_qal);
  end if;

  -- Stage 5: recompute closures (unchanged)
  if v_stage = 'recompute_closures' then
    select dst_id into v_dst_project_id from public.map_project where job_id = p_job_id and src_id = v_root_project_id;
    v_rows := public.rebuild_project_quest_closures_batch(v_dst_project_id, (select created_at from public.clone_job where id = p_job_id), p_batch_size);
    if v_rows > 0 then
