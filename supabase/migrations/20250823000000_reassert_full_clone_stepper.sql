-- purpose: reassert full multi-stage clone stepper and integrate batched recompute stage
-- notes:
-- - restores public.perform_clone_step with all stages: seed_project → clone_quests → clone_assets → clone_acl → recreate_links → recompute_closures
-- - integrates batching helpers introduced in 20250822180000_batched_project_rollup.sql
-- - preserves advisory locking, idempotent mappings, and trigger suppression via app.clone_mode
-- - ensures cloner membership is set on seed (owner) to surface cloned project in UI
...
declare
  ...
  lrec record;
begin
  -- prevent concurrent workers on the same job
  ...
  -- disable side-effect triggers during clone
  ...
  if v_stage = 'seed_project' then
    if not exists (select 1 from public.map_project where job_id = p_job_id and src_id = v_root_project_id) then
      ...
      -- ensure cloner is owner of cloned project so it appears in UI
  ...
  -- ensure dst project id is available for subsequent stages
  ...
  -- Stage 2: clone_assets (only those referenced by quests)
  if v_stage = 'clone_assets' then
    ...
    for arec in (
      ...
      values (p_job_id, arec.id, qrec.id)
      on conflict do nothing;
      v_rows := v_rows + 1;
    end loop;
    if v_rows < p_batch_size then
  ...
  -- Stage 3: clone_acl (asset_content_link)
  if v_stage = 'clone_acl' then
    v_rows := 0;
    for arec in (
      select s.*
      from public.asset_content_link s
  ...
  -- Stage 4: recreate_links (quest_asset_link, then quest_tag_link, then asset_tag_link)
      ...
      for lrec in (
        ...
        if exists (select 1 from public.map_quest mq where mq.job_id = p_job_id and mq.src_id = lrec.quest_id) then
          ...
          where mq.job_id = p_job_id and mq.src_id = lrec.quest_id
            and not exists (
              select 1 from public.quest_tag_link t where t.quest_id = mq.dst_id and t.tag_id = lrec.tag_id
            );
          get diagnostics v_last = row_count;
          v_rows := v_rows + v_last;
        end if;
    ...
    if v_rows = 0 then
      for lrec in (
        ...
        where ma.job_id = p_job_id and ma.src_id = lrec.asset_id
          and not exists (
            select 1 from public.asset_tag_link t where t.asset_id = ma.dst_id and t.tag_id = lrec.tag_id
          );
        get diagnostics v_last = row_count;
        v_rows := v_rows + v_last;
      end loop;
  ...
  -- Stage 5: recompute_closures (batched)
  if v_stage = 'recompute_closures' then
    -- ensure quest_closure exists for all dst quests
    insert into public.quest_closure (quest_id, project_id)
    select q.id, q.project_id
    from public.quest q
    where q.project_id = v_dst_project_id
    on conflict (quest_id) do nothing;

    -- rebuild outdated quest_closure rows in batches since job creation
    ...
    -- roll up project_closure in batches, larger multiplier for throughput
    ...
    -- finalize once batches are exhausted
