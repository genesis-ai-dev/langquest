...
create index if not exists asset_content_link_text_search_idx 
  on asset_content_link using gin(text_search_vector);

-- Function to update text_search_vector from text column
create or replace function asset_content_link_update_text_search_vector()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists asset_content_link_text_search_vector_trigger on asset_content_link;
create trigger asset_content_link_text_search_vector_trigger
  before insert or update of text on asset_content_link
  for each row
  execute function asset_content_link_update_text_search_vector();
