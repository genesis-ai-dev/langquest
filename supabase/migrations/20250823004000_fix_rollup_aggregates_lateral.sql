-- purpose: fix aggregate errors by avoiding set-returning functions in aggregate args
-- notes: rewrites public.rebuild_project_closure_batch to use LATERAL jsonb_array_elements_text
...
begin
  ...
  set
    ...
    total_assets = coalesce(pc.total_assets, 0) + coalesce((
      select sum(b.total_assets) from batch b
    ),0),
    total_translations = coalesce(pc.total_translations, 0) + coalesce((
      select sum(b.total_translations) from batch b
    ),0),
    approved_translations = coalesce(pc.approved_translations, 0) + coalesce((
      select sum(b.approved_translations) from batch b
    ),0),
    last_updated = now()
