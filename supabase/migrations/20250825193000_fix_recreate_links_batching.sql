-- purpose: fix recreate_links batching to only select missing destination links
-- behavior: preserves stage order and gating; only narrows batch sources
...
begin
  ...
  if v_stage = 'recreate_links' then
    ...
    -- quest_asset_link: process only missing destination links
    for lrec in (
      select qal.*
