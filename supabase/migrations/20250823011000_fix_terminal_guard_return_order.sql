-- purpose: fix terminal guard to ensure status update commits before returning
-- problem: return statement prevented the status update from committing
-- solution: ensure the update happens and commits before returning
...
begin
  ...
  -- terminal guard: if already done, ensure status is correct then return
  if v_stage = 'done' then
    -- always update status to done when stage is done (idempotent)
    update public.clone_job
    set status = 'done', updated_at = now()
    where id = p_job_id and status != 'done';  -- only update if needed
