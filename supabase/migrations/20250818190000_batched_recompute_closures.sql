-- migration: batch quest closure recompute to avoid edge runtime timeouts
-- purpose: replace global recompute with incremental, per-quest batches within perform_clone_step
...
-- Replace perform_clone_step recompute stage with batched quest rebuild, then single project rebuild
...
begin
  ...
  if v_stage = 'recompute_closures' then
    -- Process a limited number of quest closures for the destination project
    select dst_id into v_dst_project_id from public.map_project where job_id = p_job_id and src_id = v_root_project_id;
    ...
    -- All per-quest closures updated; rebuild project-level closure once
