-- purpose: fix race condition where perform_clone_step sets status='running' after stage='done' is already set
-- problem: terminal guard runs after status='running' is set, causing jobs to flip back to running
-- solution: move terminal guard before any status mutation and make it truly atomic
...
begin
  ...
  -- read current state WITHOUT mutating anything yet
  ...
  -- terminal guard: if already done, return immediately without any updates
  if v_stage = 'done' then
    -- ensure status is correct if somehow it got corrupted
  ...
  -- only set status='running' for non-terminal states
  ...
  -- Stage 5: recompute_closures (ATOMIC TERMINAL TRANSITION)
  if v_stage = 'recompute_closures' then
    ...
    -- ATOMIC: set both stage='done' AND status='done' in single transaction
    update public.clone_job 
